...
SELECT EMPNO, ENAME, JOB, MGR, SAL, GRADE||'등급' GARDE
    FROM EMP, SALGRADE
...
SELECT ENAME, SAL, GRADE||'등급' GRADE, (SAL+NVL(COMM,0))*12 연봉, DNAME
    FROM EMP E, DEPT D, SALGRADE
    WHERE SAL BETWEEN LOSAL AND HISAL
    ORDER BY DNAME, 연봉;
---	이름, 업무, 급여, 등급, 부서코드, 부서명 출력. 급여가 1000~3000사이. 정렬조건 : 부서별, 부서같으면 업무별, 업무같으면 급여 큰순
SELECT ENAME, JOB, SAL, GRADE||'등급' GRADE, E.DEPTNO 
    FROM EMP E, DEPT D, SALGRADE
    WHERE SAL BETWEEN LOSAL AND HISAL
    AND SAL BETWEEN 1000 AND 3000
    ORDER BY DNAME, JOB, SAL DESC;
---	이름, 급여, 등급, 입사일, 근무지. 81년에 입사한 사람. 등급 큰순
SELECT ENAME, SAL,  GRADE||'등급' GRADE, HIREDATE, LOC 
    FROM EMP E, DEPT D, SALGRADE
    WHERE E.DEPTNO = D.DEPTNO
    AND SAL BETWEEN LOSAL AND HISAL
...
AND WORKER.MGR = MANAGER.EMPNO;
    -- EX. 모든 사원의 사번, 이름, 사아의 사번, 상사이름
SELECT W.EMPNO, W.ENAME, W.MGR, M.ENAME
    FROM EMP W, EMP M
    WHERE W.MGR=M.EMPNO;
    -- EX. 'SCOTT의 상사는 JONES이다.' 포맷으로 출력
SELECT W.ENAME ||'의 상사는 ' || M.ENAME || '이다' MESSAGE
    FROM EMP W, EMP M
    WHERE W.MGR = M.EMPNO;
    
    -- 탄탄1 : 메니저가 KING인 사원들의 이름과 직급을 출력하시오
...
-- ★ 4. OUTER JOIN -- self join, equi join 시 조건이 만족하지 않는 행까지 나타나게 하는 join
----------------------    
...
SELECT ENAME, E.DEPTNO, DNAME FROM EMP E RIGHT JOIN DEPT D

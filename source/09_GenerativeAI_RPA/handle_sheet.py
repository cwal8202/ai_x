...
def handle_init_sheet(file_path, wb):
  # 2. 백업(파일명 : genai_rap2507011258.xlsx)
  tiemstamp = datetime.datetime.now().strftime("%y%m%d%H%M")
  backupfile = f"genai_rpa{tiemstamp}.xlsx"
  ...
  if 'now_list' in sheet_names:
    ...
    print('now_list 시트 prev_list 시트로 복사 완료')
  else :
...
def update_now_report(wb, analysis, summary):
    ...
    if 'now_report' in sheet_names:
        ...
        print('now_report 리스트가 없어서 작업을 중단합니다.')
        wb.close() # 파일 닫고 작업 중단
        return
    # 분석글(analysis, summray)을 now_report 시트에 업데이트
